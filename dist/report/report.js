...
module.exports = class Report {
  ...
  copyData(outdir) {
    ...
    return fs.pathExists(outdir).then(exists => {
      ...
      if (this.json.data.images != null) {
        this.json.data.images.forEach(img => {
          ...
          delete img.path;
          return fs.pathExists(fromPath).then(exists => {
            if (exists) {
  ...
  saveJson(outdir) {
    ...
    return fs.pathExists(outdir).then(exists => {
      ...
      return aceReport;
    }).then(aceReport => Promise.all([fs.writeFile(path.join(outdir, 'report.json'), aceReport, 'UTF-8')]));
  }
  saveHtml(outdir) {
    ...
    .then(() => fs.copy(path.join(__dirname, './resources/js/'), path.join(outdir, "js/"))).then(() => fs.writeFile(path.join(outdir, 'js/', 'aceReportData.js'), js, 'UTF-8')).catch(err => winston.error(err));
  }
};