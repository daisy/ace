...
const a11yMeta = ["schema:accessMode", "schema:accessibilityFeature", "schema:accessibilityHazard", "schema:accessibilitySummary", "schema:accessModeSufficient", "schema:accessibilityAPI", "schema:accessibilityControl", "a11y:certifiedBy"];

module.exports = {
  // each report is content doc level
  analyze: function analyze(metadata) {
    ...
    a11yMeta.forEach(function (property) {
      if (property in metadata) {
        if (typeof metadata[property] === 'string' && metadata[property].trim().length > 0 || Array.isArray(metadata[property]) && metadata[property].length > 0) {
