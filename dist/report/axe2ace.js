...
function axe2ace(spineItem, axeResults) {
  ...
  const assertion = new builders.AssertionBuilder().withSubAssertions().withTestSubject(spineItem.relpath, spineItem.title);
  // process axe's individual checks for a single content document
  axeResults.violations.forEach(violation => {
    const kbURL = kbMap.map.hasOwnProperty(violation.id) ? kbMap.baseUrl + kbMap.map[violation.id].url : kbMap.baseUrl;
    const kbTitle = kbMap.map.hasOwnProperty(violation.id) ? kbMap.map[violation.id].title : 'Unknown';
    ...
    violation.nodes.forEach(node => assertion.withAssertions(new builders.AssertionBuilder().withAssertedBy('aXe').withMode('automatic').withTest(test).withResult(new builders.ResultBuilder('fail').withDescription(node.failureSummary).withPointer(node.target, node.targetCFI).build()).build()));
  });
