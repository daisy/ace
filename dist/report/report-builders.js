...
function calculateResult(assertions) {
  ...
  assertions.forEach(assertion => {
    if ('earl:result' in assertion && assertion['earl:result']['earl:outcome'] === 'fail') {
      outcome = 'fail';
    }
  });
...
class ReportBuilder {
  constructor(title = 'Ace Report', description = 'Report on automated accessibility checks for EPUB') {
    this._json = {
      '@type': 'earl:report',
  ...
  withProperties(properties) {
    ...
    Object.getOwnPropertyNames(properties).forEach(key => {
      this._json.properties[key] = key in this._json.properties ? this._json.properties[key] || Boolean(properties[key]) : Boolean(properties[key]);
    });
    return this;
  }
