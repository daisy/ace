...
function consolidate(results, report) {
  ...
  const headings = [].concat(...results.map(docResult => docResult.outlines.headings)).filter(e => e !== undefined);
...
module.exports.check = function check(epub, report) {
  return epubChecker.check(epub, report).then(() => htmlChecker.check(epub)).then(results => consolidate(results, report));
};