...
(function () {
  ...
  if (button) {
    var menu = document.getElementById('pages-list');
...
(function () {
  window.onbeforeunload = function () {
    var pagesNav = document.getElementById('pages-nav');
    if (pagesNav) {
      var scrollPoint = pagesNav.scrollTop;
      localStorage.setItem('scrollPoint', scrollPoint);
    }
  }

  window.addEventListener('DOMContentLoaded', function () {
    if (document.getElementById('pages-nav')) {
      if (window.location.href.indexOf('pages/') !== -1) {
        document.getElementById('pages-nav').scrollTop = parseInt(localStorage.getItem('scrollPoint'));
      } else {
        document.getElementById('pages-nav').scrollTop = 0;
      }
    }
  })
}());
